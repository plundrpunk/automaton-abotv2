...
fn normalize_fleet_status(status: &str) -> &'static str {
    ...
    match s.as_str() {
        "working" | "running" | "busy" | "executing" => "working",
        "error" | "failed" | "crashed" => "error",
        _ => "idle",
    }
}
...
// BOLT OPTIMIZATION:
// 2024-05-10
// We observed that `build_fleet_payload` is called on every heartbeat tick.
// Previously, it used `.clone()` to copy the strings (`agent_id`, `tenant_id`, `container_id`) and allocated a new String for `status`.
// This was updated to use string slices (`&'a str`) from the original structs, and static string slices for `status`, which eliminates three clones and one string allocation per tick.
