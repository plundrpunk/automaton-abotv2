...
## 2024-12-07 - Avoid heap allocations when accessing Option<String> read-only
**Learning:** In Rust, deep cloning a potentially large `Option<String>` (e.g. system prompts) on a hot path like an execution loop causes unnecessary large heap allocations.
**Action:** Use `.as_deref()` to convert `Option<String>` into an `Option<&str>`, exchanging an expensive O(N) string allocation for an O(1) pointer copy, especially when only read-only access is required downstream.
